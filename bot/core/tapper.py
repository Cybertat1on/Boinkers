...
class Tapper:
    ...
    def info(self, message):
        ...
        info(f"<cyan>{self.session_name}</cyan> | {message}")

    def debug(self, message):
        from bot.utils import debug
        debug(f"<cyan>{self.session_name}</cyan> | {message}")

    def warning(self, message):
        from bot.utils import warning
        warning(f"<cyan>{self.session_name}</cyan> | {message}")

    def error(self, message):
        from bot.utils import error
        error(f"<cyan>{self.session_name}</cyan> | {message}")

    def critical(self, message):
        from bot.utils import critical
        critical(f"<cyan>{self.session_name}</cyan> | {message}")

    def success(self, message):
        from bot.utils import success
        success(f"<cyan>{self.session_name}</cyan> | {message}")
    ...
    def save_user_agent(self):
        ...
        if not any(session['session_name'] == self.session_name for session in self.session_ug_dict):
            ...
            logger.success(f"<cyan>{self.session_name}</cyan> | User agent saved successfully")
    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        ...
        try:
            ...
            logger.error(
                f"<cyan>{self.session_name}</cyan> | Unknown error during Authorization: 🚨 {error}")
    ...
    async def login(self, http_client: aiohttp.ClientSession, initdata):
        try:
            ...
            logger.error(f"<cyan>{self.session_name}</cyan> | Login error 🚨 {error}")
            return None, None
    ...
    async def upgrade_boinker(self, http_client: aiohttp.ClientSession):
         try:
             ...
             if resp.status == 200 and data:
                 logger.success(f"<cyan>{self.session_name}</cyan> Upgrade Boinker | Coins: <light-yellow>{'{:,}'.format(data['newSoftCurrencyAmount'])}</light-yellow> | Spins: <light-blue>{data['newSlotMachineEnergy']}</light-blue> | Rank: <magenta>{data['rank']}</magenta>")
                 return True
             else:
                 logger.info(f"<cyan>{self.session_name}</cyan> Upgrade Boinker | Not enough coins | Status: <magenta>{resp.status}</magenta>")
                 return False
        ...
        try:
            ...
            while can_elevate and completed_level < max_level:
                ...
                if resp.status == 200 and 'isWin' in data and data['isWin'] == True and 'prize' in data and 'prizeName' in data['prize']:
                    ...
                    logger.success(f"<cyan>{self.session_name}</cyan> Elevator | <magenta>Level</magenta> - <light-green>{completed_level}</light-green> | Prize: <magenta>{name}</magenta> - <light-green>{data['prize']['prizeValue']}</light-green>")
                    can_elevate = True
                    is_win = True
                    continue
    ...
    async def spin_wheel_fortune(self, http_client: aiohttp.ClientSession):
        try:
            ...
            if resp.status == 200 and 'prize' in data and 'prizeName' in data['prize']:
                ...
                if 'prizeTypeName' in data['prize']:
                    name = data['prize']['prizeTypeName']
                logger.success(f"<cyan>{self.session_name}</cyan> Wheel of Fortune | Prize: <magenta>{name}</magenta> - <light-green>{data['prize']['prizeValue']}</light-green>")
                return True
            else:
                return False
    ...
    async def spin_slot_machine(self, http_client: aiohttp.ClientSession, spins: int):
        ...
        try:
            while remaining_spins > 0:
                ...
                if resp.status == 200:
                    data = await resp.json()
                    logger.success(f"<cyan>{self.session_name}</cyan> | Spin prize: <light-blue>{data['prize']['prizeTypeName']}</light-blue> - <light-green>{data['prize']['prizeValue']}</light-green>")
...
#                     logger.info(f"<cyan>{self.session_name}</cyan> | Skipping task: {name_id} | Because it's on the blacklist")
                    continue
...
#                     logger.info(f"<cyan>{self.session_name}</cyan> | Skipping task: {name_id} | Because you need to join the group or channel.")
                    continue
                ...
                if not can_perform_task:
                    if wait_time:
                        ...
                        logger.info(f"<cyan>{self.session_name}</cyan> | Need to wait {wait_seconds} seconds to perform task {name_id}")
                    continue
                ...
                if settings.AD_TASK_PREFIX.lower() in name_id.lower():
                    ...
                    try:
                        async with http_client.post(click_url, ssl=False) as click_response:
                            ...
                            logger.info(f"<cyan>{self.session_name}</cyan> | Performed task {name_id}. Status: pending")

                    except Exception as click_error:
                        logger.error(f"<cyan>{self.session_name}</cyan> | Error performing task {name_id}: {click_error}")
                        continue
                    ...
                    if seconds_to_allow_claim > 60:
                        logger.info(f"<cyan>{self.session_name}</cyan> | Need to wait {seconds_to_allow_claim} seconds to perform task {name_id}")
                        continue

                    logger.info(f"<cyan>{self.session_name}</cyan> | 💤 Waiting {seconds_to_allow_claim} seconds before claiming reward...")
                        ...
                        async with http_client.post(claim_url, ssl=False) as claim_response:
                            if claim_response.status == 200:
                                ...
                                if result != None and 'prizeGotten' in result:
                                    ...
                                    logger.success(f"<cyan>{self.session_name}</cyan> | Successfully completed task {name_id} | Reward: 💰<light-green>{reward}</light-green>")
                            else:
                                logger.info(f"<cyan>{self.session_name}</cyan> | Failed to claim reward for {name_id}. Status code: <light-red>{claim_response.status}</light-red>")
                    except Exception as claim_error:
                        logger.info(f"<cyan>{self.session_name}</cyan> | Error claiming reward for {name_id}: {claim_error}")
                        break

                await asyncio.sleep(delay=1)

        except Exception as error:
            logger.info(f"<cyan>{self.session_name}</cyan> | Error performing tasks: 🚨 {error}")
    ...
    async def handle_ad_task(self, http_client: aiohttp.ClientSession, name_id, provider_id, action):
        try:
            ...
            logger.info(f"<cyan>{self.session_name}</cyan> | Ad task {name_id} clicked successfully")

            logger.info(f"<cyan>{self.session_name}</cyan> | 💤 Sleep 5 seconds before close ad...")
            await asyncio.sleep(delay=5)
            ...
            logger.info(f"<cyan>{self.session_name}</cyan> | Ad task {name_id} watched successfully")
            ...
            logger.info(f"<cyan>{self.session_name}</cyan> | 💤 Sleep {seconds_to_allow_claim} seconds before claiming ad reward...")
            await asyncio.sleep(delay=seconds_to_allow_claim)

            # Claim the reward
            claim_url = f"https://boink.astronomica.io/api/rewardedActions/claimRewardedAction/{name_id}?p=android"
            logger.info(f"<cyan>{self.session_name}</cyan> | Sending reward claim request for ad task {name_id}...")
            async with http_client.post(claim_url, headers=headers) as claim_response:
                if claim_response.status == 200:
                    result = await claim_response.json()
                    reward = result.get('prizeGotten')
                    logger.success(f"<cyan>{self.session_name}</cyan> | Successfully completed ad task {name_id} | Reward: 💰<light-green>{reward}</light-green>")
                else:
                    logger.error(f"<cyan>{self.session_name}</cyan> | Failed to claim reward for ad task {name_id}. Status code: {claim_response.status}")

        except Exception as error:
            logger.error(f"<cyan>{self.session_name}</cyan> | Error handling ad task {name_id}: 🚨 {error}")
    ...
    async def run(self, proxy: str | None) -> None:
        if settings.USE_RANDOM_DELAY_IN_RUN:
            ...
            logger.info(f"<cyan>{self.session_name}</cyan> | Bot will start in <ly>{random_delay}s</ly>")
        ...
        while True:
            try:
                ...
                logger.error(
                    f"<cyan>{self.session_name}</cyan> | Unknown error during login: 🚨 {error}")
                await asyncio.sleep(delay=3)
            ...
            try:
                ...
                if user_info is not None:
                    if user_info['boinkers'] and 'completedBoinkers' in user_info['boinkers']:
                        logger.info(f"<cyan>{self.session_name}</cyan> | Boinkers: <light-blue>{user_info['boinkers']['completedBoinkers']}</light-blue>")

                    if 'currencySoft' in user_info:
                        logger.info(f"<cyan>{self.session_name}</cyan> | Coin Balance: 💰 <light-green>{'{:,}'.format(user_info['currencySoft'])}</light-green>")

                    if 'currencyCrypto' in user_info:
                        logger.info(f"<cyan>{self.session_name}</cyan> | Shit Balance: 💩 <blue>{'{:,.3f}'.format(user_info['currencyCrypto'])}</blue>")
                    ...
                    if not last_claimed_time or current_time > last_claimed_time + timedelta(hours=2, minutes=5):
                        ...
                        if success:
                            logger.success(f"<cyan>{self.session_name}</cyan> | 🚀 Claimed boost successfully 🚀")
                    ...
                    if settings.ENABLE_AUTO_SPIN:
                        ...
                        if spin_user and 'gamesEnergy' in spin_user and 'slotMachine' in spin_user['gamesEnergy']:
                            ...
                            logger.info(f"<cyan>{self.session_name}</cyan> | Spins: <light-blue>{spins}</light-blue>")
                            if spins > 0:
                ...
                logger.info(f"<cyan>{self.session_name}</cyan> | 💤 sleep 30 minutes")
                await asyncio.sleep(delay=1800)

            except Exception as error:
                logger.error(
                    f"<cyan>{self.session_name}</cyan> | 😢 Unknown error: 🚨 {error}")
